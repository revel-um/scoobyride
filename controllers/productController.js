    ...
    Store.findById(storeId)
        ...
        .then((result) => {
            ...
            for (const key of keys) {
                if (key != "productImages" && key != "bookings") productObj[key] = req.body[key];
            }
    ...
    Store.findById(storeId)
        ...
        .then((result1) => {
            if (result1 == undefined)
                res.status(400).json({ message: "This store does not exist" });
...
exports.getAllProducts = (req, res, next) => {
    ...
    if (searchPurelyOnLocation === "true") {
        ...
        if (lat0 == null || lon0 == null || km == null) {
            Product.find()
                .populate({ path: "store", match: { city: city } })
                .exec()
                .then((result) => {
                    returnObj = [];
                    for (r of result) {
...
exports.getProductsByQuery = (req, res, next) => {
    ...
    if (searchText == null)
        return res
            .status(200)
            .json({ message: "searchText is a required parameter for searching" });
    if (city == null)
        return res.status(200).json({ message: "City is a required parameter for searching" });
        ...
        .then((result) => {
            if (result != null) {
                ...
                for (const key of Object.keys(req.body)) {
                    if (key != "bookings") updateObj[key] = req.body[key];
                }
                if (req.body.bookings) {
                    const a = JSON.parse(req.body.bookings);
                    productObj["bookings"] = a;
                }
...
exports.deleteImageFromProduct = (req, res, next) => {
    ...
    Product.findById(id)
        ...
        .then((result) => {
            ...
            Product.updateOne({ _id: id }, { $set: { productImages: allLinks } })
                .exec()
                .then((result) => {
                    res.status(200).json({ data: result });
                })
...
exports.updateRatings = (req, res, next) => {
    ...
    Product.findById(id)
        ...
        .then((result) => {
            ...
            Product.updateOne({ _id: id }, { $set: { ratings: newRating, ratedBy: num + 1 } })
                .exec()
                .then((result) => {
